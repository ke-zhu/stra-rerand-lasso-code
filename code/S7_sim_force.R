...
toc()
save(Z_design_rep, file = str_glue("output/result/force_des_c{job_id}.RData"))
...
inf_tab <- map2_dfr(Z_design_rep, c("no", "yes"), function(Z_rep, rerand) {
  ...
  mclapply(Z_rep, function(Z) {
    ...
    rbind(
      ...
      stra_adj(Y, Z, B, X, "lasso (force adj)", force_adj = 1:5)
    )
  }, mc.cores = n_cores) %>% 
    map_dfr(~.) %>% 
...
toc()
save(inf_tab, file = str_glue("output/result/force_inf_c{job_id}.RData"))
...
toc()
save(sum_tab, file = str_glue("output/force_sum_c{job_id}.RData"))
