...
setups0 <- expand_grid(
  ...
  s          = 200,
  SNR        = 5,
  X_cov      = "highly corr" # c("highly corr", "toeplitz")
) %>% 
...
simdata <- map(1:nrow(setups0), ~ {
  ...
  if (X_cov == "highly corr") {
    ...
    size_group <- 3
    n_group <- floor(s / size_group)
    X <- matrix(rnorm(n * p), n, p)
    for (i in 0:(n_group - 1)) {
      W <- rnorm(n)
      for (j in 1:size_group) {
        X[,i * size_group + j] <- W + rnorm(n, 0, sqrt(0.1))
      }
    }
  ...
  beta01 <- map(1:2, ~ c(
    runif(s, -0.1, 0.1), 
    #c(rep(0.5, 5), rep(0.75, 5), rep(1, 5)),
    #rt(10, 3),
    rep(0, p - s)
  ))
  for (trt in c(0, 1)) {
...
data_sum <- imap_dfr(simdata, ~ {
  ...
  out <- tibble(
    case = .y,
    tau = mean(Y1) - mean(Y0),
    varY1 = round(var(Y1), 2), 
    varY0 = round(var(Y0), 2), 
    R2_1_50 = round(summary(lm(Y1 ~ X[,1:50]))$r.squared, 1),
    R2_1_200 = round(summary(lm(Y1 ~ X[,1:200]))$r.squared, 1)
  )
  detach(.x)
  out
})
...
save(setups, simdata, file = "data/sim_corr.RData")
save(setups, file = "data/setups_corr.RData")
