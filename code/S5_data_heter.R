...
#   })
# })
# fx <- map2_dbl(1:n, B, function(.x, .y) {
#   sum((X - 2 * X_sc)[.x,] * beta[[.y]])
# })
...
setups0 <- expand_grid(
  ...
  s          = 10,
  SNR        = 5,
  X_cov      = c("toeplitz") # c("highly corr", "toeplitz")
) %>% 
...
save(setups, simdata, file = "data/sim_heter.RData")
save(setups, file = "data/setups_heter.RData")
